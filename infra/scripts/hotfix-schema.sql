...
CREATE TABLE IF NOT EXISTS `prepayment` (
  ...
  `statement_url` VARCHAR(1000) DEFAULT NULL COMMENT '对账单附件(2026-09-20 #145 供应商对账单,图片/PDF/Excel,逗号分隔多份)',
  `created_at`     DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`     DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_factory` (`factory_id`)
...
CALL _i9_add_col('sample_garment','image1',"VARCHAR(1000) DEFAULT NULL COMMENT '图片槽1(多图片/PDF/Excel,逗号分隔;下游取首图继承)'");
CALL _i9_sync_col('sample_garment','image1',"VARCHAR(1000)","VARCHAR(1000) DEFAULT NULL COMMENT '图片槽1(多图片/PDF/Excel,逗号分隔;下游取首图继承)'");
CALL _i9_add_col('sample_garment','image2',"VARCHAR(1000) DEFAULT NULL COMMENT '图片槽2(多图片/PDF/Excel,逗号分隔;下游取首图继承)'");
...
CALL _i9_add_col('prepayment','slip_url',"VARCHAR(500) DEFAULT NULL COMMENT '银行水单(2026-09-11 #134 预付也是真实付款,要留凭证)'");
CALL _i9_sync_col('prepayment','slip_url',"VARCHAR(500)","VARCHAR(500) DEFAULT NULL COMMENT '银行水单(2026-09-11 #134 预付也是真实付款,要留凭证)'");
CALL _i9_add_col('prepayment','statement_url',"VARCHAR(1000) DEFAULT NULL COMMENT '对账单附件(2026-09-20 #145 供应商对账单,图片/PDF/Excel,逗号分隔多份)'");
CALL _i9_sync_col('prepayment','statement_url',"VARCHAR(1000)","VARCHAR(1000) DEFAULT NULL COMMENT '对账单附件(2026-09-20 #145 供应商对账单,图片/PDF/Excel,逗号分隔多份)'");
...
CALL _i9_add_col('payment_request','related_style_no',"VARCHAR(200) DEFAULT NULL COMMENT '相关款号(结算按款号归集;宽度须>=reconciliation.style_no的200,否则带入时截断)'");
CALL _i9_sync_col('payment_request','related_style_no',"VARCHAR(200)","VARCHAR(200) DEFAULT NULL COMMENT '相关款号(结算按款号归集;宽度须>=reconciliation.style_no的200,否则带入时截断)'");
...
-- feedback
CALL _i9_add_col('feedback','user_id',"BIGINT       NOT NULL COMMENT '提交用户(内部用户ID 或 供应商账号ID,按 user_type 区分)'");
CALL _i9_sync_col('feedback','user_id',"BIGINT","BIGINT       NOT NULL COMMENT '提交用户(内部用户ID 或 供应商账号ID,按 user_type 区分)'");
CALL _i9_add_col('feedback','user_type',"ENUM('INTERNAL','SUPPLIER') NOT NULL DEFAULT 'INTERNAL' COMMENT '提交人来源;两套ID各自独立,不加此列会串号'");
CALL _i9_sync_col('feedback','user_type',"ENUM('INTERNAL','SUPPLIER')","ENUM('INTERNAL','SUPPLIER') NOT NULL DEFAULT 'INTERNAL' COMMENT '提交人来源;两套ID各自独立,不加此列会串号'");
