...
test_reconciliation() {
  ...
  # 整单退回：业务提交→主管退回→回草稿+批注（补充确认：可逐批批注、整单退回）
  api PATCH "/reconciliations/${bid:-0}/submit" '' "$TOKEN_BUSINESS"
  expect_ok "空白对账单提交复核"
  api PATCH "/reconciliations/${bid:-0}/reject" '{"remark":"金额需核对"}' "$TOKEN_SUPERVISOR"
  expect_ok "主管整单退回"
  api GET "/reconciliations/${bid:-0}"
  expect_eq data.status DRAFT "退回后回草稿DRAFT"
  expect_eq data.review_remark "金额需核对" "退回批注已记录"
