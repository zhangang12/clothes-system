...
CREATE TABLE IF NOT EXISTS `settlement` (
  `id`                   BIGINT         NOT NULL AUTO_INCREMENT,
  `js_no`                VARCHAR(20)    NOT NULL COMMENT 'JS-YYYYMMDD-001',
  `order_id`             BIGINT         NOT NULL,
  `shipped_qty`          INT            NOT NULL COMMENT '出货件数',
  `currency`             VARCHAR(5)     NOT NULL DEFAULT 'USD',
  `exchange_rate`        DECIMAL(10,4)  DEFAULT NULL COMMENT '汇率',
  `revenue`              DECIMAL(15,4)  NOT NULL COMMENT '总货款(CNY)',
  `cost_total`           DECIMAL(15,4)  NOT NULL COMMENT '总成本',
  `cost_per_unit`        DECIMAL(15,4)  DEFAULT NULL COMMENT '成本单价=总成本÷出货件数',
  `gross_profit`         DECIMAL(15,4)  DEFAULT NULL COMMENT '毛利=货款-成本',
  `gross_margin`         DECIMAL(5,2)   DEFAULT NULL COMMENT '毛利率%',
  `tax_refund`           DECIMAL(15,4)  NOT NULL DEFAULT 0 COMMENT '退税金额',
  `net_profit`           DECIMAL(15,4)  DEFAULT NULL COMMENT '净利(含退税)=毛利+退税',
  `net_profit_ex_refund` DECIMAL(15,4)  DEFAULT NULL COMMENT '净利(不含退税)=毛利',
  `status`               ENUM('DRAFT','CONFIRMED') NOT NULL DEFAULT 'DRAFT',
  `confirmed_by`         BIGINT         DEFAULT NULL COMMENT '确认人（FINANCE）',
  `confirmed_at`         DATETIME       DEFAULT NULL,
  `remark`               TEXT           DEFAULT NULL,
  `created_by`           BIGINT         NOT NULL,
  `created_at`           DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `settlement_cost` (
  ...
  `reconcile_id`    BIGINT         NOT NULL COMMENT '关联对账单',
  `has_invoice`     TINYINT        NOT NULL DEFAULT 0 COMMENT '1=有票',
  `invoice_amount`  DECIMAL(15,4)  DEFAULT NULL COMMENT '发票金额（有票时）',
  `tax_rate`        DECIMAL(5,2)   DEFAULT NULL COMMENT '税率%（有票时）',
  `cost_excl_tax`   DECIMAL(15,4)  DEFAULT NULL COMMENT '不含税成本（有票时）',
  `payment_amount`  DECIMAL(15,4)  DEFAULT NULL COMMENT '付款金额（无票时全额）',
  `cost`            DECIMAL(15,4)  NOT NULL COMMENT '最终成本（有票=不含税 无票=付款全额）',
  `item_type`       VARCHAR(50)    DEFAULT NULL COMMENT '费用类型（面料/加工费等）',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `settlement_receipt` (
  `id`             BIGINT        NOT NULL AUTO_INCREMENT,
  `settlement_id`  BIGINT        NOT NULL,
  `receipt_date`   DATE          NOT NULL COMMENT '收汇日期',
  `amount_foreign` DECIMAL(15,4) NOT NULL COMMENT '外币金额',
  `currency`       VARCHAR(5)    NOT NULL DEFAULT 'USD',
  `exchange_rate`  DECIMAL(10,4) NOT NULL COMMENT '汇率',
  `amount_cny`     DECIMAL(15,4) NOT NULL COMMENT '实收人民币',
  `remark`         TEXT          DEFAULT NULL,
  `created_by`     BIGINT        NOT NULL,
  `created_at`     DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
