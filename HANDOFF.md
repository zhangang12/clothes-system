...
**注**:下游报价/合同列表的客户行级过滤(设计稿「在报价/合同中均不可见」)尚未接——下拉已过滤(新单据选不了未授权客户),存量单据列表尚显示客户名,待后续接 quote/order findAll 过滤。
**审计清单至此全部完成**——所有设计稿大项(合同编辑页/订单矩阵/门户对账/付款分批/客户机密)均已交付并真栈验证。
