...
describe('CSV 解析', () => {
  ...
  it('B148 字段中间的引号（5" 拉链）当普通字符，不能进引号态把后文整行吞掉', () => {
    expect(parseCsv('品名,颜色\n5" 拉链,黑色\n普通料,白色')).toEqual([
      ['品名', '颜色'],
      ['5" 拉链', '黑色'],
      ['普通料', '白色'],
    ]);
  ...
  it('B148 字段开头的引号仍按包裹处理（"帽子,大身" 不拆列）', () => {
    expect(parseCsv('品名,部位\nA,"帽子,大身"')).toEqual([['品名', '部位'], ['A', '帽子,大身']]);
  });

  it('B148 尺寸写成 20"×30" 这种两个引号也不串行', () => {
    const rows = parseCsv('品名,尺寸\n包装袋,20"×30"\n下一行,正常');
    expect(rows[1]).toEqual(['包装袋', '20"×30"']);
    expect(rows[2]).toEqual(['下一行', '正常']);
  });
