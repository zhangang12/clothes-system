...
describe('报价单导出 Excel', () => {
  ...
  it('BOM + 款号强制文本', async () => {
    const html = await exportedHtml(() => exportQuoteExcel(base));
  ...
  it('费用金额 = 单价 × 数量，数量缺省按 1（与 quotePrint 口径一致）', async () => {
    const html = await exportedHtml(() => exportQuoteExcel({
      ...base,
  ...
  it('无费用时不输出空的费用明细表', async () => {
    expect(await exportedHtml(() => exportQuoteExcel(base))).not.toContain('费用明细');
  });

  it('无报价明细时给占位而不是空表', async () => {
    expect(await exportedHtml(() => exportQuoteExcel({ quote_no: 'Q1' }))).toContain('（无报价明细）');
  });
});
