...
describe('CustomerEditView', () => {
  ...
  // ── B100：客户的数字字段清空后发 undefined，付款期限、佣金率清不掉 ──
  it('B100 清空付款期限/默认佣金率后保存发 null（发 undefined 后端当「不改」，重开还在）', async () => {
    mockRoute.params = { id: '5' };
    mockCustomerGet.mockResolvedValue({ data: makeCustomer({ payment_days: 30, commission_rate: 2.5 }) });
    const wrapper: any = mountView();
    await vi.waitFor(() => expect(wrapper.vm.form.commissionRate).toBe(2.5));
    expect(wrapper.vm.buildDto().paymentDays).toBe(30);
    expect(wrapper.vm.buildDto().commissionRate).toBe(2.5);

    wrapper.vm.form.paymentDays = '';
    wrapper.vm.form.commissionRate = undefined;
    const dto = wrapper.vm.buildDto();
    expect(dto.paymentDays).toBeNull();
