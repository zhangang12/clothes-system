...
export class ContractService {
  ...
  private expandMaterialLines(
    ...
    const base = {
      ...
      // 拉链三件套是合同快照的一部分：供应商合同按真实版式要各自成列印出来，
      // 拼进 spec 就没法单独排版了（2026-08-08 用户给的真实合同格式）
      puller: om.puller || undefined,
      zipper_teeth: om.zipper_teeth || undefined,
      code_band: om.code_band || undefined,
      unit: om.unit,
    ...
    // 用 7 个字符的 BY_BOTH 规避，不动表结构。**要改名先看列宽。**
    if ((mode === 'BY_COLOR' || mode === 'BY_SIZE' || mode === 'BY_BOTH') && matrixRows?.length) {
      const keyOf = (r: any): { key: string; color: string; size: string } => {
        const color = String(r?.color ?? '').trim();
        const size = String(r?.size ?? '').trim();
        if (mode === 'BY_COLOR') return { key: color, color, size: '' };
        if (mode === 'BY_SIZE') return { key: size, color: '', size };
        // 颜色与尺码都要有才成组，缺一个就说明这行矩阵数据不完整，跳过比拼出"黑色|"这种空维度好
        // 【别用 `${color} ${size}` 这类拼串】颜色里本来就带空格（导入的色号形如
        // 「10 White 增白色」），拼出来会有歧义；更别用不可见分隔符——曾误写成裸 NUL 字节，
        // 结果整个文件被 git/grep 判成二进制、diff 和搜索全废。JSON 化最稳且仍是纯文本。
        return { key: color && size ? JSON.stringify([color, size]) : '', color, size };
      };
      const groups = new Map<string, number>();
      const dims = new Map<string, { color: string; size: string }>();
      for (const r of matrixRows) {
        const { key, color, size } = keyOf(r);
      ...
      if (groups.size) {
        ...
        return [...groups].map(([key, groupQty]) => {
          ...
          const sizeKey = mode === 'BY_COLOR' ? '' : (mode === 'BY_SIZE' ? key : d.size);
          const spec = sizeKey ? (om.size_specs?.[sizeKey] || '').trim() : '';
          return {
            ...base,
  ...
  async findAll(query: QueryContractDto) {
    ...
    // 关键词搜索：合同号 / 关联款号 / 供应商名（用户反馈 2026-08-07 King：「合同号记不住」）。
    // 供应商名不在 contract 表上（只有 factory_id），先按名字查一遍工厂拿 id 再并进 OR——
    // 这样不必把整个 findAndCount 改写成 QueryBuilder，分页/排序的既有行为一个字不动。
    let where: FindOptionsWhere<Contract> | FindOptionsWhere<Contract>[] = base;
    if (keyword) {
      const kw = String(keyword).trim();
      const branches: FindOptionsWhere<Contract>[] = [
        { ...base, contract_no: Like(`%${kw}%`) },
        { ...base, style_nos: Like(`%${kw}%`) },
      ];
      // 空关键词已被外层 if 挡掉；这里再挡一次纯空白，否则 LIKE '%%' 会把全表工厂捞进来
      if (kw) {
        const factories = await this.dataSource.getRepository(Factory).find({
          where: { name: Like(`%${kw}%`), deleted: 0 },
          select: ['id'],
          take: 200, // 关键词太泛时别把 IN 列表撑爆
        });
        if (factories.length) branches.push({ ...base, factory_id: In(factories.map((f) => f.id)) });
      }
      where = branches;
    }
