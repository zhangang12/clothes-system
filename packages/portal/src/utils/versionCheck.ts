...
/**
 * 发版后让浏览器主动换到新版本（2026-08-11）。与 packages/web/src/utils/versionCheck.ts 是同一套逻辑，
 * 两个包之间没有共享运行时代码（types 包只放类型），故各留一份；改一处记得改另一处。
 *
...
async function check(): Promise<void> {
  ...
  showNotify({
    type: 'primary',
    message: '系统已更新，点此立即刷新',
    duration: 0, // 不自动消失：让用户自己决定什么时候刷（可能正在填发货单）
