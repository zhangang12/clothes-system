// 门户合同打印 —— 浏览器原生打印（A4，电脑登录可直出纸质/另存 PDF）
// 与 PC 端 utils/contractPrint.ts 同一套版式：材料/加工各套模板；甲乙方按类型；条款 terms_json 优先模板兜底；
// 数据源为门户详情接口返回的 contract + factory(本厂) + company(本司默认主体)。
...
export function printContract(
  ...
  // 甲乙双方块（材料:甲方=供方/乙方=本司；加工:受托方=工厂/委托方=本司）
  const partyBlock = `
  <div class="meta">
  ...
  const stampState = detail.stamped_at
    ...
    : '（尚未盖章——打印本页纸质盖章后，拍照回传门户「纸质盖章照片」）';
  // 电子章贴图：已电子盖章且工厂库配置章图→贴甲方/受托方落款；本司章有配置即贴
  ...
  * { box-sizing: border-box; }
  ...
  <div class="sign">
    ...
    <div>
      ...
      <div class="stamp-hint">${company?.seal_url ? '✓ 本司电子章已贴入落款' : ''}</div>
